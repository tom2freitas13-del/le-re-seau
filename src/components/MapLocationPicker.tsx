...
export default function MapLocationPicker({ initialPosition, onConfirm, onClose }: MapLocationPickerProps) {
  ...
  const selectedPointLabel = (lat: number, lng: number) => t('mapLocationPicker.selectedPoint', { lat: lat.toFixed(4), lng: lng.toFixed(4) });

  const handlePick = async (lat: number, lng: number) => {
    setPicked({ lat, lng });
    setResolving(true);
    const found = await reverseGeocode(lat, lng);
    setLabel(found || selectedPointLabel(lat, lng));
    setResolving(false);
  };

  const handleConfirm = () => {
    if (!picked) return;
    onConfirm(label || selectedPointLabel(picked.lat, picked.lng), picked.lat, picked.lng);
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-background flex flex-col">
      <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-md border-b border-border/50 px-4 py-3 flex items-center justify-between">
        <h2 className="font-display text-lg font-semibold">{t('mapLocationPicker.title')}</h2>
      ...
      <div className="flex-1 relative">
        <ErrorBoundary fallbackTitle={t('mapLocationPicker.mapErrorTitle')} fallbackMessage={t('mapLocationPicker.mapErrorMessage')}>
      ...
      <div className="sticky bottom-0 bg-background/95 backdrop-blur-md border-t border-border/50 px-4 py-3 safe-area-bottom">
        {picked ? (
          <p className="text-sm mb-2 flex items-center gap-1.5" style={{ fontFamily: 'Jost, sans-serif' }}>
            {resolving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : '📍'} {resolving ? t('mapLocationPicker.resolvingName') : label}
          </p>
        ) : (
          <p className="text-sm text-muted-foreground mb-2" style={{ fontFamily: 'Jost, sans-serif' }}>
            {t('mapLocationPicker.tapMap')}
          </p>
        )}
